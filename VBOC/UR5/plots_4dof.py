...
def plots_4dof(X_save, q_min, q_max, v_min, v_max, model_dir, mean_dir, std_dir, device):
    ...
    with torch.no_grad():
        ...
        for i in range(inp.shape[0]):
            vel_norm = norm([inp[i,4],inp[i,5],inp[i,6],inp[i,7]])
            inp[i][0] = (inp[i][0] - mean_dir) / std_dir
            inp[i][1] = (inp[i][1] - mean_dir) / std_dir
            inp[i][2] = (inp[i][2] - mean_dir) / std_dir
            inp[i][3] = (inp[i][3] - mean_dir) / std_dir
            if vel_norm != 0:
                inp[i][4] = inp[i][4] / vel_norm
                inp[i][5] = inp[i][5] / vel_norm
                inp[i][6] = inp[i][6] / vel_norm
                inp[i][7] = inp[i][7] / vel_norm
            inp[i][8] = vel_norm
        out = (model_dir(torch.from_numpy(inp[:,:8].astype(np.float32)).to(device))).cpu().numpy() 
        ...
        #         and norm(X_save[i][6]) < 0.1
        #     ):
        #         xit.append(X_save[i][3])
        #         yit.append(X_save[i][7])
        # plt.plot(
        #     xit,
        #     yit,
        #     "ko",
        ...
        #         norm(X_save[i][0] - (q_min + q_max) / 2) < 0.1 and
        #         norm(X_save[i][1] - (q_min + q_max) / 2) < 0.1 and
        #         norm(X_save[i][3] - (q_min + q_max) / 2) < 0.1
        #         and norm(X_save[i][4]) < 0.1
        #         and norm(X_save[i][5]) < 0.1
        #         and norm(X_save[i][7]) < 0.1
        #     ):
        #         xit.append(X_save[i][2])
        #         yit.append(X_save[i][6])
        # plt.plot(
        #     xit,
        #     yit,
        #     "ko",
        ...
        #     if (
        #         norm(X_save[i][0] - (q_min + q_max) / 2) < 0.1 and
        #         norm(X_save[i][2] - (q_min + q_max) / 2) < 0.1 and
        #         norm(X_save[i][3] - (q_min + q_max) / 2) < 0.1
        #         and norm(X_save[i][4]) < 0.1
        #         and norm(X_save[i][6]) < 0.1
        #         and norm(X_save[i][7]) < 0.1
