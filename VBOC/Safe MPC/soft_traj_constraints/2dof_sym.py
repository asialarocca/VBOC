...
ocp.ocp_solver.cost_set(ocp.N, "Zl", 1e6*np.ones((1,)))
...
for i in range(res_steps.shape[0]):
    if res_steps_term[i]-res_steps[i]>0:
        ...
        worse += 1
plt.title('MPC standard vs MPC with soft traj constraints')
...
np.save('res_steps_softtrajconstr.npy', np.array(res_steps_term).astype(int))
