    ...
    for f in range(tot_steps):
        ...
        if failed_iter == 0 and f > 0:
            for i in range(ocp.N):
                if ocp.nn_decisionfunction(params, mean_dir, std_dir, safety_margin, ocp.ocp_solver.get(i+1, 'x')) >= 0.:
                    receiding = ocp.N - i

        for i in range(ocp.N + 1):
            ocp.ocp_solver.cost_set(i, "Zl", np.ones((1,)))

        ocp.ocp_solver.cost_set(ocp.N-failed_iter-receiding, "Zl", 1e6*np.ones((1,)))
...
# Pytorch params:
device = torch.device("cpu") # pytorch device #"cuda" if torch.cuda.is_available() else 
...
np.save('res_steps_hardreceidingconstr.npy', res_steps_term)
