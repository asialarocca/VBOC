...
eps = 1e-3 # unviable data generation parameter

tol = 1e-4 # OCP cost tolerance
...
# Save training data:
np.save('data_2dof_vboc_10.npy', np.asarray(X_save))

# Load training data:
# X_save = np.load('data_2dof_vboc_10.npy')

# # Print other data generations statistics:
# for k in range(4):
...
# Load the model:
# model_dir.load_state_dict(torch.load('model_2dof_vboc_10'))
...
# # Filter the training data to reduce the number of states with low velocities:
...
# Save the model:
torch.save(model_dir.state_dict(), 'model_2dof_vboc_10')
