...
print("\t%d iter, %.1f s, loss %.4f (loss thr %.4f)"%(it, time.time()-time_training_start,
                                                    val, loss_stop))
...
while (pos_new <= pos_old or iterbreak > 10) and iterbreak < 150 and time.time() - start_time - init_times - test_times < max_time:
    ...
    print("Training finished after %d iter, %.1f s, loss %.4f (loss thr %.4f)"%(it, 
                                                                                time.time()-time_training_start,
                                                                                val, loss_stop))
    ...
    # plt.plot(loss_hst)
    # plt.title('NN loss evolution')
    # plt.ylabel('Loss')
    # plt.xlabel('Iteration')
    # plt.yscale('log')
    # plt.grid(True, which="both")
    # plt.savefig("nn_training_loss_iter_%3d"%(iterbreak), format='png')
...
torch.save(model.state_dict(), 'model_2dof_hjr_10_1800s')
torch.save(mean, 'mean_2dof_hjr_10')
torch.save(std, 'std_2dof_hjr_10')

print('RMSE test data: ', math.sqrt(np.sum(np.power(norm_error_al,2))/len(norm_error_al))) 

np.save('times_hjr.npy', np.asarray(times))
np.save('rmse_hjr.npy', np.asarray(rmse))
