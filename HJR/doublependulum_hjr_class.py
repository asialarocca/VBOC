...
class OCPdoublependulum:
    ...
    def compute_problem(self, x0):
        ...
        x_guess = np.array([x0[0]+x0[2]*1e-2, x0[1]+x0[3]*1e-2, x0[2]*0.9, x0[3]*0.9])
        u_guess = np.array([self.g*self.l1*(self.m1+self.m2)*math.sin(x0[0]),self.g*self.l2*self.m2*math.sin(x0[1])])
