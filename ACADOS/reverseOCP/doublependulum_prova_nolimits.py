...
if __name__ == "__main__":
    ...
    for _ in range(num):
        ...
        ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_min + 1e-4, q_min, v_min, v_min, 1.]))
        ocp.ocp_solver.constraints_set(0, "ubx", np.array([q_min + 1e-4, q_max, v_max, v_max, 1.]))
        ...
        if status == 0:
            ...
            for f in range(1, ocp.N):
                ...
                if x_sol[f][0] > q_max - 1e-4 or x_sol[f][0] < q_min + 1e-4 or x_sol[f][1] > q_max - 1e-4 or x_sol[f][1] < q_min + 1e-4 or x_sol[f][2] > v_max - 1e-4 or x_sol[f][2] < v_min + 1e-4 or x_sol[f][3] > v_max - 1e-4 or x_sol[f][3] < v_min + 1e-4:
                    is_x_at_limit_now = True
                else:
                    is_x_at_limit_now = False

                if is_x_at_limit_now == True: # sono su dX
                    ...
                    if is_x_at_limit_before == True: # sono o su dV o in V, risolvi un nuovo OCP
                        ...
                        if status == 0:
                            if ocp.ocp_solver.get(0, "x")[2] == x_sol[f][2]:
                                ...
                                #ricordati comunque di salvare is on limit before e robe varie
                        else:
                            break
            ...
            plt.title('Controls, X0 at the limit = ' + str(is_x_at_limit))
            plt.grid()
            plt.subplot(2, 1, 2)
            ...
            plt.title('States, X0 at the limit = ' + str(is_x_at_limit))
            plt.grid()
            plt.subplot(4, 1, 2)
