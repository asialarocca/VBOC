...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        #mean, std = torch.tensor(1.9635), torch.tensor(7.0253) # max vel = 15

    # Initialization of the SVM classifier:
    clf = svm.SVC(C=1e6, kernel='rbf', class_weight='balanced')

    modell = NeuralNet(input_size, hidden_size, output_size).to(device)

    loss_stop = 0.2  # nn training stopping condition
    beta = 0.8
    n_minibatch = 512
    it_max = 1e5
    ...
    for _ in range(num):
        ...
        ocp.ocp_solver.constraints_set(ocp.N, "lbx", np.array([q_max, q_fin, 0., 0., 1e-2]))
        ...
        for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
            x_guess = np.array([(1-tau)*q_min + tau*q_max, q_fin, v_max, 0., 1e-2]) # 2*(1-tau)*(q_max-q_min)
            ocp.ocp_solver.set(i, 'x', x_guess)
            ocp.ocp_solver.set(i, 'p', p)
            ocp.ocp_solver.constraints_set(i, 'lbx', np.array([q_min, q_min, 0., v_min, 1e-2])) 
            ocp.ocp_solver.constraints_set(i, 'ubx', np.array([q_max, q_max, v_max, v_max, 1e-2])) 

        ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_min + eps, q_min, 0., v_min, 1e-2]))
        ...
        if status == 0:
            ...
            ocp.ocp_solver.constraints_set(0, 'lbx', np.array([x0[0], x0[1], x0[2], x0[3], 1e-2])) 
            ...
            for f in range(1, ocp.N):
                ...
                if is_x_at_limit:
                    if x_sol[f][0] > q_max - eps or x_sol[f][0] < q_min + eps or x_sol[f][1] > q_max - eps or x_sol[f][1] < q_min + eps or x_sol[f][2] > v_max - eps or x_sol[f][2] < v_min + eps or x_sol[f][3] > v_max - eps or x_sol[f][3] < v_min + eps:
                        ...
                        if x_sol[f-1][2] > v_max - eps: # or x_sol[f-1][0] < q_min + eps:
                            print('Distacco dal limite avvenuto su v1, risolvo un nuovo OCP')
                            ...
                            # if x_sol[f-1][3] > v_max - eps or x_sol[f-1][1] < q_min + eps:
                            #     print('Distacco dal limite avvenuto su v2, risolvo un nuovo OCP')
                            ...
                            # else:
                            #     if x_sol[f-1][3] < v_min + eps or x_sol[f-1][1] > q_max - eps:
                            #         print('Distacco dal limite avvenuto su v2, risolvo un nuovo OCP')
                ...
                #     if is_x_at_limit == False: #etp < 1e-2 and out_v[0,1] > 0:
                ...
                #         ocp.ocp_solver.options_set('qp_tol_stat', 1e-4)
                ...
                #         if status == 0:
                #             if abs(ocp.ocp_solver.get(0, "x")[2] - x_sol[f][2]) > 1e-4:
        ...
        # ocp.ocp_solver.constraints_set(ocp.N, "lbx", np.array([q_min, q_fin, 0., 0., 1e-2]))
        ...
        # for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
        #     x_guess = np.array([(1-tau)*q_max + tau*q_min, q_fin, -2*(1-tau)*(q_max-q_min), 0., 1e-2]) # v_min
        #     ocp.ocp_solver.set(i, 'x', x_guess)
        #     ocp.ocp_solver.set(i, 'p', p)
        ...
        # ocp.ocp_solver.constraints_set(0, "ubx", np.array([q_max - eps, q_max, 0., v_max, 1e-2]))
        ...
        #                 if x_sol[f-1][2] < v_min + eps or x_sol[f-1][0] > q_max - eps:
        #                     print('Distacco dal limite avvenuto su v1, risolvo un nuovo OCP')
        ...
        # ocp.ocp_solver.constraints_set(ocp.N, "lbx", np.array([q_fin, q_max, 0., 0., 1e-2]))
        ...
        # for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
        #     x_guess = np.array([q_fin, (1-tau)*q_min + tau*q_max, 0., 2*(1-tau)*(q_max-q_min), 1e-2]) # v_max
        #     ocp.ocp_solver.set(i, 'x', x_guess)
        #     ocp.ocp_solver.set(i, 'p', p)
        #     ocp.ocp_solver.constraints_set(i, 'lbx', np.array([q_min, q_min, v_min, 0., 1e-2])) 
        #     ocp.ocp_solver.constraints_set(i, 'ubx', np.array([q_max, q_max, v_max, v_max, 1e-2])) 

        # ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_min, q_min + eps, v_min, 0., 1e-2]))
        ...
        #                     if ocp.ocp_solver.get(0, "x")[3] == x_sol[f][3]:
        #                         print('Sono su dV, ottenuto tramite la nuova soluzione')
        #                         print(x_sol[f], x_sym)
        #                         is_x_at_limit = False
        #                         xv_state[f] = 0
        ...
        #                         # sono su dv, posso simulare
        #                         x_sym = np.copy(x_sol[f][:4])
        #                         x_sym[3] = x_sym[3] - eps * p[3] / (2*v_max)
        ...
        # ocp.ocp_solver.constraints_set(ocp.N, "lbx", np.array([q_fin, q_min, 0., 0., 1e-2]))
        ...
        # for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
        #     x_guess = np.array([q_fin, (1-tau)*q_max + tau*q_min, 0.,  -2*(1-tau)*(q_max-q_min), 1e-2]) # v_min
        #     ocp.ocp_solver.set(i, 'x', x_guess)
        #     ocp.ocp_solver.set(i, 'p', p)
        #     ocp.ocp_solver.constraints_set(i, 'lbx', np.array([q_min, q_min, v_min, v_min, 1e-2])) 
        ...
        # ocp.ocp_solver.constraints_set(0, "ubx", np.array([q_max, q_max - eps, v_max, 0., 1e-2]))
        ...
        #                     ocp.ocp_solver.constraints_set(0, 'lbx', np.array([x_sol[f][0], x_sol[f][1], x_sol[f][2], v_min, dt_sym])) 
        #                     ocp.ocp_solver.constraints_set(0, 'ubx', np.array([x_sol[f][0], x_sol[f][1], x_sol[f][3], x_sol[f][3], dt_sym])) 
        #                 else:
        #                     if x_sol[f-1][2] > v_max - eps or x_sol[f-1][0] < q_min + eps:
        #                         print('Distacco dal limite avvenuto su v1, risolvo un nuovo OCP')
    ...
    # meanl, stdl = torch.mean(torch.Tensor(X_train[:,:4])).to(device), torch.std(torch.Tensor(X_train[:,:4])).to(device)
    ...
    #     X_iter_tensor = (X_iter_tensor - meanl) / stdl
    ...
    # data_al = np.load('data.npy')

    # data_boundary = np.array([data_al[i] for i in range(len(data_al)) if abs(model((torch.Tensor(data_al[i,:4]) - mean) / std)[0]) < 5])
