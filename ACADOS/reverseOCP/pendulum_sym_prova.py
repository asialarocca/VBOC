...
if __name__ == "__main__":
    ...
    ocp.model.cost_expr_ext_cost_0 = weight * dtheta / dthetamax + weight * theta / (thetamax -  thetamin)
    ocp.model.cost_expr_ext_cost = 0.
    ocp.model.cost_expr_ext_cost_e = 0.
    ocp.parameter_values = np.array([-1.])
    ...
    if status == 0:
        ...
        for f in range(N):
            current_val = ocp_solver.get(f, "x")
            X_plot = np.append(X_plot, [[current_val[0], current_val[1]]], axis = 0)
    ...
    plt.figure()
    plt.scatter(X_plot[:,0], X_plot[:,1], c = 'black', marker=".")
    h = 0.01
