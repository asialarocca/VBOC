...
np.save('data3_vbocp_10_50000.npy', np.asarray(X_save))
# X_save = np.load('data3_vbocp_10_100000.npy')
...
#X_train_dir = X_save_dir[:int(len(X_save_dir)*0.7)]
#X_test_dir = X_save_dir[int(len(X_save_dir)*0.7):]
X_train_dir = np.copy(X_save_dir)

# model_dir.load_state_dict(torch.load('model_3pendulum_dir_10_100000'))
...
torch.save(model_dir.state_dict(), 'model_3pendulum_dir_10_50000')
...
# outputs = np.empty((len(X_test_dir),1))
# n_minibatch_model = pow(2,15)
...
#             outputs[n_minibatch_model*idx:len(X_test_dir)] = model_dir(batch).cpu().numpy()
#         else:
#             outputs[n_minibatch_model*idx:n_minibatch_model*(idx+1)] = model_dir(batch).cpu().numpy()

#     outputs_tensor = torch.Tensor(outputs).to(device)
#     print('RMSE test data: ', torch.sqrt(criterion_dir(outputs_tensor, y_iter_tensor))) 
