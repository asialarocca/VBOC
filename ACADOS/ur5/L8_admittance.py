...
import tf
import numpy as np
from double_pendulum_ocp_class_mpc import OCPdoublependulumNN, OCPdoublependulumINIT
from UR5_ocp_class_mpc import OCPUR5INIT
...
def talker(p):
    ...
    print('PRE loadModelAndPublishers\n')
    p.loadModelAndPublishers(xacro_path)
    print('loadModelAndPublishers\n')
    p.initVars()
    print('initVars\n')
    p.startupProcedure()
    ...
    if conf.robot_params[p.robot_name]["control_mode"] == "trajectory":
        ...
        while True:
            ...
            # ocp.compute_problem(p.q, p.qd)
            # current_val = ocp.ocp_solver.get(1, "x")

            # print('p_des:', current_val)

            # p.q_des = current_val[:6]
            # p.qd_des = current_val[6:]

            # current_contr = ocp.ocp_solver.get(1, "u")
            # p.tau_ffwd = current_contr
            ...
            if lab_conf.admittance_control:
                ...
                p.x_ee_des = p.robot.framePlacement(p.q_des, p.robot.model.getFrameId(
                    conf.robot_params[p.robot_name]['ee_frame'])).translation
                p.q_des_adm, p.x_ee_des_adm = p.admit.computeAdmittanceReference(
                    p.contactForceW, p.x_ee_des, p.q)
            ...
            if (p.time > 1.5) and (
                lab_conf.admittance_control
            ):  # activate admittance control only after a few seconds to allow initialization
