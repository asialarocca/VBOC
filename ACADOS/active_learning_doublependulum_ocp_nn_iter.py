...
with cProfile.Profile() as pr:
    ...
    #     n_sum = sum(
    #         np.argmax(model((torch.Tensor(data[:pow(10, ocp_dim)])-mean)/std).numpy(), axis=1))  # used for the stopping criterion
    ...
    #     # Ocp initialization:
    #     ocp = OCPdoublependulumNN(model, mean, std)
    ...
    #     for i in range(len(X_iter)):
    #         if y_iter[i] == [0, 1]:
    #             datax.append(X_iter[i])
    #             datay.append(y_iter[i])
    #         else:
    #             dataxu.append(X_iter[i])

    #     with torch.no_grad():
    #         for i in range(len(Xu_iter)):
    #             if np.argmax(model((torch.Tensor(Xu_iter[i])-mean)/std).numpy()) == 0:
    #                 dataxu.append(X_iter[i])
    ...
    #         ind = random.sample(range(len(X_iter) - N_init), int(n_minibatch / 2))
    #         ind.extend(
    #             random.sample(
    #                 range(len(X_iter) - N_init, len(X_iter)),
    #                 int(n_minibatch / 2),
    #             )
    #         )
    ...
    #         n_temp = sum(
    #             np.argmax(model((torch.Tensor(data[:pow(10, ocp_dim)])-mean)/std).numpy(), axis=1))
    #         if n_temp - n_sum == 0:
    #             break
    #         n_sum = n_temp
