...
with torch.no_grad():
    ...
    out = model(torch.from_numpy(np.c_[xx.ravel(), yy.ravel()].astype(np.float32)))
    y_pred = np.argmax(out.numpy(), axis=1)
    Z = y_pred.reshape(xx.shape)
    z = [0 if np.array_equal(y_iter[x], [1, 0]) else 1 for x in range(y_iter.shape[0])]
...
while not (etpmax < etp_stop or Xu_iter.shape[0] == 0):
    ...
    selec = [i for i in range(X_iter.shape[0] - size) if np.random.uniform() <= 1 / k]
    selec.extend([i for i in range(X_iter.shape[0] - size, X_iter.shape[0])])
