...
import time
from double_pendulum_ocp_class_mintime import OCPdoublependulumRINIT
import warnings
...
with cProfile.Profile() as pr:
    ...
    for i in range(num):
        ...
        ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_min, q_min, 0., v_min, 0.]))
        ocp.ocp_solver.constraints_set(0, "ubx", np.array([q_min, q_max, v_max, v_max, 1e-2]))

        ran = random.choice([-1, 1]) * random.random() * multip

        ocp.ocp_solver.reset()

        for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
            x_guess = np.array([(1-tau)*q_min + tau*q_max, q_fin, v_max, 0., 1e-2])
            ocp.ocp_solver.set(i, 'x', x_guess)
            ocp.ocp_solver.set(i, 'p', np.array([-1., ran]))
        ...
        if status == 0:
            for f in range(ocp.N+1):
                current_val = ocp.ocp_solver.get(f, "x")[:4]
    ...
    for i in range(num):     
        ...
        ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_max, q_min, v_min, v_min, 0.]))
        ...
        for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
            x_guess = np.array([tau*q_min + (1-tau)*q_max, q_fin, v_min, 0., 1e-2])
            ocp.ocp_solver.set(i, 'x', x_guess)
            ocp.ocp_solver.set(i, 'p', np.array([1., ran]))
    ...
    for i in range(num):
        ...
        ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_min, q_min, v_min, 0., 0.]))
        ...
        for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
            x_guess = np.array([q_fin, (1-tau)*q_min + tau*q_max, 0., v_max, 1e-2])
            ocp.ocp_solver.set(i, 'x', x_guess)
            ocp.ocp_solver.set(i, 'p', np.array([ran, -1.]))
    ...
    for i in range(num):     
        ...
        ocp.ocp_solver.constraints_set(0, "lbx", np.array([q_min, q_max, v_min, v_min, 0.]))
        ...
        for i, tau in enumerate(np.linspace(0, 1, ocp.N, endpoint=False)):
            x_guess = np.array([q_fin, tau*q_min + (1-tau)*q_max, 0., v_min, 1e-2])
            ocp.ocp_solver.set(i, 'x', x_guess)
            ocp.ocp_solver.set(i, 'p', np.array([ran, 1.]))
