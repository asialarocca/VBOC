...
with cProfile.Profile() as pr:
    ...
    torch.save(mean, 'mean_2pendulum_10')
    torch.save(std, 'std_2pendulum_10')
    ...
    while not (etpmax < etp_stop or len(Xu_iter) == 0) and time.time() - start_time < 1800:
    ...
    torch.save(model.state_dict(), 'model_2pendulum_al_10_1800')
    np.save('data2_al_10_1800.npy', np.asarray(X_iter))

    print(time.time() - start_time)
    print('RMSE test data: ', math.sqrt(np.sum(np.power(norm_error_al,2))/len(norm_error_al))) 

    np.save('times_al.npy', np.asarray(times))
    np.save('rmse_al.npy', np.asarray(rmse))
