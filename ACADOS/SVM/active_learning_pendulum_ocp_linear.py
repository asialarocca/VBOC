...
with cProfile.Profile() as pr:
    ...
    if show_plots:
        # Plot the results:
        plt.figure()
        data_transformed = feature_map_nystroem.transform(np.c_[xx.ravel(), yy.ravel()])
    ...
    while not (etpmax < etp_stop or Xu_iter.shape[0] == 0):
        ...
        data_transformed = feature_map_nystroem.transform(Xu_iter)
        prob_xu = clf.predict_proba(data_transformed)
        ...
        data_transformed = feature_map_nystroem.fit_transform(X_iter)
        clf.fit(data_transformed, y_iter)
