...
with cProfile.Profile() as pr:
    ...
    stop_time = 14400 # total computational time

    # Models and optimizer:
    model = NeuralNet(input_size, hidden_size, output_size).to(device)
    model_guess = NeuralNet(input_size, hidden_size, ocp_dim*ocp.N).to(device)
    ...
    torch.save(mean, 'mean_3dof_al_10')
    torch.save(std, 'std_3dof_al_10')
    ...
    np.save('times_3dof_al.npy', np.asarray(times))
    np.save('rmse_3dof_al.npy', np.asarray(rmse))
    torch.save(model.state_dict(), 'model_3dof_10_14400s')
    np.save('data_3dof_al_10_14400s.npy', np.asarray(X_iter))
